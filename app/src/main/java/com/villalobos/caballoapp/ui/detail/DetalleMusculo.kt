...
class DetalleMusculo : BaseNavigationActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            ErrorHandler.handleError(
                ...
                canRecover = true,
                recoveryAction = { navigateBackToRegion(regionId) }
            )
        }
    }

    private fun setupUI() {
        binding.btnVolver.setOnSafeClickListener {
            navigateBackToRegion(regionId)
        }
        ...
        viewModel.state.observe(this) { state ->
            if (state.hasValidData) {
                state.musculo?.let { musculo ->
                    ...
                    binding.imgMusculoDetalle.contentDescription = getString(
                        R.string.muscle_detail_image_description,
                        musculo.nombre
                    )
    ...
    private fun configurarImagenMusculo(imageName: String?) {
        ErrorHandler.safeExecute(
            ...
            val imageResource = DrawableNameResolver.resolve(imageName)
            val fallbackImageName = DatosMusculares.obtenerImagenFallbackPorRegion(regionId)
            val fallbackImageResource = DrawableNameResolver.resolve(fallbackImageName)

            if (imageResource != 0) {
                binding.imgMusculoDetalle.setImageResource(imageResource)
            } else if (fallbackImageResource != 0) {
                binding.imgMusculoDetalle.setImageResource(fallbackImageResource)
            } else {
