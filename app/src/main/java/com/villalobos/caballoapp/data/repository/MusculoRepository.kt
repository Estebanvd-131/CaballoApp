...
class MusculoRepository @Inject constructor(
    ...
    private val canonicalMusclesById: Map<Int, Musculo> by lazy {
        DatosMusculares.obtenerTodosLosMusculos().associateBy { it.id }
    }

    private fun mergeWithCanonicalData(musculo: Musculo): Musculo {
        return canonicalMusclesById[musculo.id] ?: musculo
    }
    ...
    fun getAllMusclesFlow(): Flow<List<Musculo>> {
        return musculoDao.getAllMusculos().map { entities ->
            entities.map { entity ->
                mergeWithCanonicalData(entity.toModel())
            }
        }
    }
    ...
    suspend fun getMuscleByIdAsync(id: Int): Musculo? {
        return musculoDao.getMusculoById(id)?.toModel()?.let(::mergeWithCanonicalData)
    }
    ...
    suspend fun getAllMusclesAsync(): List<Musculo> {
        return musculoDao.getAllMusculosSync().map { entity ->
            mergeWithCanonicalData(entity.toModel())
        }
    }
    ...
    suspend fun getMusclesByRegionAsync(regionId: Int): List<Musculo> {
        return musculoDao.getMusculosByRegionSync(regionId).map { entity ->
            mergeWithCanonicalData(entity.toModel())
        }
    }
    ...
    suspend fun getMuscleByIdUnified(id: Int): Musculo? {
        return getMuscleByIdAsync(id)
            ?: DatosMusculares.obtenerMusculoPorId(id)
            ?: canonicalMusclesById[id]
    }
    ...
    fun getMuscleById(id: Int): Musculo? {
        return DatosMusculares.obtenerMusculoPorId(id) ?: canonicalMusclesById[id]
    }
